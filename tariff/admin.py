...
class Notes_sectionAdmin(admin.ModelAdmin):
    ...
    search_fields = ('id', 'note_a', 'note_b')
    # autocomplete_fields = ['id_section']  # ✅ This now works!
...
class Notes_chapterAdmin(admin.ModelAdmin):
    ...
    search_fields = ('id', 'note_a', 'note_b', 'note_c', 'note_d', 'note_e')
...
class Notes_subchapterAdmin(admin.ModelAdmin):
    ...
    search_fields = ('id', 'note_a', 'additional_note', 'name_a' , 'name_addition')
...
class HsCodeAdmin(admin.ModelAdmin):
    # form = HsCodeForm 
    list_display = ('edit_link','parent_3','item' ,'label', 'import_fee', 'ser_all','export_fee','exp_se_Fe', 'type','active','full_import_fee')
    search_fields = ('hs_code', 'label',)
    list_filter = ('id_parent_3', )
    ordering = ('hs_code',)
    list_editable = ('import_fee','export_fee','ser_all','active','exp_se_Fe',)
    readonly_fields = ('review','review_en','review_value')
    inlines = [ ImportFeeInline, ExportFeeInline, StoneFarmingInline]

    def save_model(self, request, obj, form, change):
        for field in form.fields:
            if not form.cleaned_data.get(field):
                setattr(obj, field, None)  # ✅ Allow empty fields
    ...
    def parent_3(self, obj):
        return obj.id_parent_3   #Use the instance, not the class

    class Media:
        css = {
            'all': (static('/admin/css/custom_admin.css'),)  # ✅ Use Django static tag
        }
        js = (static('/admin/js/custom_admin.js'),)
    def edit_link(self, obj):
        """Return a clickable pen icon that opens a popup for editing."""
        edit_url = f"/admin/tariff/hscode/{obj.id}/change/"
        return format_html(
