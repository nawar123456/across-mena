...
from .models import ExportFee, Finance,Stone_Farming,Commercial_Description, Image_Description, Fee, ImportFee, Notes_chapter, Notes_fee, Notes_section, Notes_subchapter, Section, Chapter, Sub_Chapter, HsCode
...
class NotesSectionSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        # Helper to safely replace None with empty string
        def safe(value):
            return value if value is not None else ""

        # Apply safe conversion to all note fields
        data['note_a'] = safe(data.get('note_a'))
        data['note_b'] = safe(data.get('note_b'))
        data['note_c'] = safe(data.get('note_c'))
        data['note_num'] = safe(data.get('note_num'))
        data['note_a_en'] = safe(data.get('note_a_en'))
        data['note_b_en'] = safe(data.get('note_b_en'))
        data['note_c_en'] = safe(data.get('note_c_en'))
        data['note_num_en'] = safe(data.get('note_num_en'))
        ...
        # Remove English fields always (frontend shouldn't care about them)
...
class NotesChapterSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        # Apply safe fallback for all fields involved
        data['note_a'] = safe(data.get('note_a'))
        data['note_b'] = safe(data.get('note_b'))
        data['note_c'] = safe(data.get('note_c'))
        data['note_d'] = safe(data.get('note_d'))
        data['note_e'] = safe(data.get('note_e'))
        data['note_num'] = safe(data.get('note_num'))
        data['note_a_en'] = safe(data.get('note_a_en'))
        data['note_b_en'] = safe(data.get('note_b_en'))
        data['note_c_en'] = safe(data.get('note_c_en'))
        data['note_d_en'] = safe(data.get('note_d_en'))
        data['note_e_en'] = safe(data.get('note_e_en'))
        data['note_num_en'] = safe(data.get('note_num_en'))
        ...
        # Remove English fields from final output
        data.pop('note_a_en', None)
        data.pop('note_b_en', None)
        data.pop('note_c_en', None)
...
class CustomFeeSerializer(serializers.ModelSerializer):
    ...
    def get_commercial_descriptions(self, obj):
        commercial_descriptions = getattr(obj, 'filtered_descriptions', [])
        return CommercialDescriptionSerializer(commercial_descriptions, many=True).data
