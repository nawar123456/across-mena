...
from .models import ExportFee, Finance,Stone_Farming,Commercial_Description, Image_Description, Chapter, Fee, ImportFee, Notes_chapter, Notes_fee, Notes_section, Notes_subchapter, Section, Sub_Chapter
...
def get_fees_for_sub_chapter(request, sub_chapter_id):
    try:
        ...
        fees = Fee.objects.filter(id_parent_3=sub_chapter)
...
class SearchView(APIView):
    def get(self, request):
        ...
        id_pattern = re.escape(search_string)
        
        # Search for Fee instances
        fee_results = Fee.objects.filter(Q(id__iregex=id_pattern) | Q(label__iregex=pattern) | Q(review__iregex=pattern) | Q(label_en__iregex=pattern))

        # Filter Commercial_Description instances based on the search pattern
        commercial_desc_results = []
        if search_string:
            commercial_desc_results = Commercial_Description.objects.filter(Q(second_description__iregex=pattern))

        # Collect all fee IDs associated with the found Commercial_Description instances
        fee_ids_from_commercial_desc = [commercial_desc.id_desc_id for commercial_desc in commercial_desc_results]

        # Fetch Fee instances associated with the collected fee IDs
        fee_results_from_commercial_desc = Fee.objects.filter(id__in=fee_ids_from_commercial_desc)

        # Combine all Fee results
        all_fee_results = fee_results | fee_results_from_commercial_desc

        # Serialize Fee results
        serialized_results = {
            "Fee": []
        }

        # If there are Fee results, serialize them
        if all_fee_results:
            for fee_result in all_fee_results:
                # Check if there are matching commercial descriptions
                matching_commercial_desc = Commercial_Description.objects.filter(id_desc_id=fee_result.id, second_description__iregex=pattern).exists()
                serialized_fee = CustomFeeSerializer(fee_result, context={'request': request}).data
                if matching_commercial_desc:
                    # Include commercial descriptions if they exist
                    serialized_fee["commercial_descriptions"] = CommercialDescriptionSerializer(Commercial_Description.objects.filter(id_desc_id=fee_result.id), many=True).data
                else:
                    # Set commercial_descriptions to an empty array if no matches found
                    serialized_fee["commercial_descriptions"] = []
                serialized_results["Fee"].append({
                    "model_name": "Fee",
                    "data": [serialized_fee]
                })
