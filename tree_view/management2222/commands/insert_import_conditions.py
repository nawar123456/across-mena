...
from tree_view.models import HsCode, ImportFee
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        csv_file_path = os.path.join(os.path.dirname(__file__), 'import_conditions.csv')
            ...
            for row in reader:
                ...
                try:
                    import_fee, created = ImportFee.objects.get_or_create(
                        id=row['id'],
                        defaults={
                            'restriction_import_en': row['restriction_import_en'],
                            'document_import':row['document_import'],
                            'document_import_en':row['document_import_en'],
